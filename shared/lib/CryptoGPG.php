...
class CryptoGPG 
{
     ...
     * @param  bool $secret Whether to export the secret key or not
     * @return mixed  false on error, the key block with the exported keys
     */
    function Export($KeyID = false, $secret = false)
    {
        ...
        $contents = '';
        if ($secret)
            $secKeyStr = "-secret-key";
        // execute the GPG command
        if ( $this->_fork_process($this->program_path . ' --homedir ' . $this->home_directory .
                    ' --armor --export'.$secKeyStr.' ' . $KeyID,
                    false, $contents) )
