...
var HistoryManager = {
	 ...
	 * @param	{Array} default values, the input values given to onMatch and onGenerate will be complemente
	...
	register: function(key, defaults, onMatch, onGenerate, regexp, options) {
		...
		data.onGenerate = data.onGenerate || function(values) { return key + '-' + values.join(','); };
	...
	setState: function(state, fix) {
		...
		if (Browser.Engine.trident && (!fix || this.istateOld)) {
			...
			try {
				var doc = this.iframe.contentWindow.document;
				doc.open();
				doc.write('<html><body id="state" state="'+state+'">' + state + '</body></html>');
				doc.close();
...
HistoryManager.extend(new Events($empty)).extend(new Options($empty));
