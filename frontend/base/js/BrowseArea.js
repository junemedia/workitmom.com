var BrowseArea = new Class({
	...
	addUpdateEvents: function() {
		...
		this.container.getElements('div#browse_bar a, div.pagination a, a.tag').each(function(link) {
			link.addEvent('click', this.onLinkClick.bindWithEvent(this, link));
		}, this);
		
		/* And a little something special for tab links... */
		this.container.getElements('a.tab').each(function(tab){
			
			/* Add link click event handler */
			tab.addEvent('click', this.onLinkClick.bindWithEvent(this, tab));
			...
			if (href[1] && $(href[1])){
				...
				/* Set scroll target */
				this.setOptions({scrollTo: href[1]});
		...
		/* Renable pageScroll */
		new PageScroll(this.container.getElements('a.scroll'), {
			wheelStops: false
		});
