...
var sIFR = new function() {
  ...
  function UserAgentDetection(sIFR) {
    ...
    this.ie               = Browser.Engine.trident;
    this.ieSupported      = this.ie         && !/ppc|smartphone|iemobile|msie\s5\.5/.test(ua) && (Browser.Engine.version >= 4);
  ...
  this.replace = function(kwargs, mergeKwargs) {
    ...
    for(var i = 0; i < nodes.length; i++) {
      ...
      //console.log([lines, size, height, renderHeight]);
      
      // Fix node height to prevent jumping whilst we inject
  ...
  function calculate(node) {
    ...
    if(!ua.ie) { //:=todo Only do once for each selector?
      ...
      if(fontSize.indexOf('px') > 0) {
        ...
        lines = Math.floor(node.getHeight() / fontSize);
      } else {
